    ...
    <main className="min-h-screen">
      <section className="bg-primary text-primary-foreground py-16 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <p className="text-secondary font-sans uppercase tracking-[0.3em] text-sm mb-2">Colecția Noastră</p>
          <h1 className="font-serif text-4xl md:text-5xl font-bold">
            {showWishlist ? "Lista Ta de Favorite" : "Uleiuri de Măsline Premium"}
          </h1>
        </motion.div>
      </section>
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <p className="text-sm text-muted-foreground font-sans mb-6">{filtered.length} produs{filtered.length !== 1 ? "e" : ""} găsit{filtered.length !== 1 ? "e" : ""}</p>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-20">
            <p className="font-serif text-2xl text-muted-foreground">Niciun produs găsit</p>
            <p className="text-sm text-muted-foreground font-sans mt-2">Încearcă să ajustezi filtrele sau termenul de căutare.</p>
          </div>
        )}
      </div>
    </main>
